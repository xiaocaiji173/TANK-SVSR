...
class PerceptualLoss(tnn.Module):
    def __init__(self, requires_grad=False):
        ...
        self.slice1 = tnn.Sequential()
        self.slice2 = tnn.Sequential()
        self.slice3 = tnn.Sequential()
        self.slice4 = tnn.Sequential()
        for x in range(2):
    ...
    def __call__(self, im1, im2, mask=None, conf_sigma=None):
        ...
        for f1, f2 in feats[2:3]:  # use relu3_3 features only
            ...
            if mask is not None:
                ...
                sh, sw = hm//h, wm//w
                mask0 = tnn.functional.avg_pool2d(mask, kernel_size=(sh,sw), stride=(sh,sw)).expand_as(loss)
